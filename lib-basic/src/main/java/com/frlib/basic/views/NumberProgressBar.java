...
public class NumberProgressBar extends View {
    ...
    public NumberProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mReachedBarColor = attributes.getColor(R.styleable.NumberProgressBar_npb_reached_color, defaultReachedColor);
        int defaultUnreachedColor = Color.rgb(204, 204, 204);
        mUnreachedBarColor = attributes.getColor(
                R.styleable.NumberProgressBar_npb_unreached_color,
                defaultUnreachedColor
        );
        int defaultTextColor = Color.rgb(66, 145, 241);
        mTextColor = attributes.getColor(R.styleable.NumberProgressBar_npb_text_color, defaultTextColor);
        mTextSize = attributes.getDimension(R.styleable.NumberProgressBar_npb_text_size, defaultTextSize);

        mReachedBarHeight = attributes.getDimension(
                R.styleable.NumberProgressBar_npb_reached_bar_height,
                defaultReachedBarHeight
        );
        mUnreachedBarHeight = attributes.getDimension(
                R.styleable.NumberProgressBar_npb_unreached_bar_height,
                defaultUnreachedBarHeight
        );
        mOffset = attributes.getDimension(R.styleable.NumberProgressBar_npb_text_offset, defaultProgressTextOffset);

        int textVisible = attributes.getInt(R.styleable.NumberProgressBar_npb_text_visibility, PROGRESS_TEXT_VISIBLE);
        if (textVisible != PROGRESS_TEXT_VISIBLE) {
            mIfDrawText = false;
        }

        setProgress(attributes.getInt(R.styleable.NumberProgressBar_npb_current, 0));
        setMax(attributes.getInt(R.styleable.NumberProgressBar_npb_max, 100));
