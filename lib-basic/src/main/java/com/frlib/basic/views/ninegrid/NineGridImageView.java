...
public class NineGridImageView<T> extends ViewGroup {
    ...
    private final static int COUNT_5 = 5;
    private final static int COUNT_6 = 6;
     ...
     */
    /** 跨行跨列的类型 - 不跨行也不跨列 */
    public final static int NOSPAN = 0;
    /** 跨行跨列的类型 - 首行跨列 */
    ...
    private AbstractNineGridImageViewAdapter<T> mAdapter;
    ...
    private void layoutChildrenView() {
        ...
        if (mSpanType == NOSPAN || showChildrenCount <= COUNT_2) {
    ...
    private void layoutForNoSpanChildrenView(int childrenCount) {
        ...
        for (int i = 0; i < childrenCount; i++) {
            ...
            if (mAdapter != null) {
                mAdapter.onDisplayImage(getContext(), childrenView, i >= mImgDataList.size() ? null : mImgDataList.get(i));
            }
        }
    }
    ...
    private void generatUnitRowAndColumnForSpanType(int imagesSize, int[] gridParam) {
        if (imagesSize <= COUNT_2) {
            gridParam[0] = 1;
            gridParam[1] = imagesSize;
        } else if (imagesSize == COUNT_3) {
            switch (mSpanType) {
                case TOPCOLSPAN:
                case BOTTOMCOLSPAN:
                case LEFTROWSPAN:
                    // 2行2列,首行跨列 2行2列,末行跨列 2行2列,首列跨行
                    gridParam[0] = 2;
                    gridParam[1] = 2;
                    break;
                case NOSPAN:
        ...
        } else if (imagesSize <= COUNT_6) {
            switch (mSpanType) {
                case TOPCOLSPAN:
                case BOTTOMCOLSPAN:
                case LEFTROWSPAN:
                    // 3行3列,首行跨列 3行3列,末行跨列 3行3列,首列跨行
                    gridParam[0] = 3;
                    gridParam[1] = 3;
                    break;
                case NOSPAN:
    ...
    public void setImagesData(List<T> lists, int spanType) {
        boolean notData = (null == lists || lists.isEmpty()) && !mNeedPlus;
        if (notData) {
    ...
    private int getNeedShowCount() {
        if (mImgDataList != null) {
            int size = mImgDataList.size() + 1;
            return Math.min(size, mMaxSize);
        }

        return mNeedPlus ? 1 : 0;
    }
    ...
    private ImageView getImageView(final int position) {
        if (position < mImageViewList.size()) {
            ...
            if (mAdapter != null) {
                ...
                Timber.e("Your must set a NineGridImageViewAdapter for NineGirdImageView");
                return null;
            }
        }
    }
     ...
     * @param adapter 适配器
     */
    public void setAdapter(AbstractNineGridImageViewAdapter<T> adapter) {
        mAdapter = adapter;
    }
     ...
     * 设置是否需要加号
     */
    public void setNeedPlus(boolean mNeedPlus) {
        this.mNeedPlus = mNeedPlus;
    }
