...
class CircleProgressView @JvmOverloads constructor(
    ...
    // 未完成进度条大小
    private var mReachBarSize = getContext().dp2px(2f)

    // 未完成进度条大小
    private var mNormalBarSize = getContext().dp2px(2f)
    ...
    // 进度值字体大小
    private var mTextSize = getContext().sp2Px(14f)
    ...
    // 画笔是否使用圆角边界，normalStyle下生效 = false
    private var mReachCapRound = false

    // 半径
    private var mRadius = getContext().sp2Px(20f)
    ...
    // 进度风格
    private var mProgressStyle = ProgressStyle.NORMAL

    // 内部圆与外部圆间距
    private var mInnerPadding = getContext().sp2Px(1f)
    ...
    // 外层圆环宽度
    private var mOuterSize = getContext().dp2px(1f)
    ...
    private fun obtainAttributes(attrs: AttributeSet?) {
        ...
        // 获取三种风格通用的属性
        mNormalBarSize =
            ta.getDimension(R.styleable.CircleProgressView_cpv_progressNormalSize, mNormalBarSize.toFloat()).toInt()
        mNormalBarColor = ta.getColor(R.styleable.CircleProgressView_cpv_progressNormalColor, mNormalBarColor)
        mReachBarSize =
            ta.getDimension(R.styleable.CircleProgressView_cpv_progressReachSize, mReachBarSize.toFloat()).toInt()
        ...
        if (ta.hasValue(R.styleable.CircleProgressView_cpv_progressTextSuffix)) {
            mTextSuffix = ta.getString(R.styleable.CircleProgressView_cpv_progressTextSuffix).invalid()
        }
        if (ta.hasValue(R.styleable.CircleProgressView_cpv_progressTextPrefix)) {
            mTextPrefix = ta.getString(R.styleable.CircleProgressView_cpv_progressTextPrefix).invalid()
        }
        ...
        when (mProgressStyle) {
            ...
            ProgressStyle.FILL_IN_ARC -> {
                ...
                mInnerPadding =
                    ta.getDimension(R.styleable.CircleProgressView_cpv_innerPadding, mInnerPadding.toFloat()).toInt()
            ...
            ProgressStyle.NORMAL -> {
                ...
                if (ta.hasValue(R.styleable.CircleProgressView_cpv_innerBackgroundColor)) {
                    mInnerBackgroundColor =
                        ta.getColor(R.styleable.CircleProgressView_cpv_innerBackgroundColor, Color.argb(0, 0, 0, 0))
    ...
    var reachBarSize: Int
        get() = mReachBarSize
        set(reachBarSize) {
            mReachBarSize = context.dp2px(reachBarSize.toFloat())
            invalidate()
        }
    ...
    var normalBarSize: Int
        get() = mNormalBarSize
        set(normalBarSize) {
            mNormalBarSize = context.dp2px(normalBarSize.toFloat())
            invalidate()
        }
    ...
    var textSize: Int
        get() = mTextSize
        set(textSize) {
            mTextSize = context.sp2Px(textSize.toFloat())
            invalidate()
        }
    ...
    var radius: Int
        get() = mRadius
        set(radius) {
            mRadius = context.dp2px(radius.toFloat())
            invalidate()
        }
    ...
    var innerPadding: Int
        get() = mInnerPadding
        set(innerPadding) {
            mInnerPadding = context.dp2px(innerPadding.toFloat())
            val mInnerRadius = mRadius - mOuterSize / 2 - mInnerPadding
            rectInner = RectF(
                (-mInnerRadius).toFloat(),
                (-mInnerRadius).toFloat(),
    ...
    var outerSize: Int
        get() = mOuterSize
        set(outerSize) {
            mOuterSize = context.dp2px(outerSize.toFloat())
            invalidate()
        }
    ...
    override fun onRestoreInstanceState(state: Parcelable) {
        if (state is Bundle) {
            ...
            mTextSuffix = bundle.getString(TEXT_SUFFIX).invalid()
            mTextPrefix = bundle.getString(TEXT_PREFIX).invalid()
